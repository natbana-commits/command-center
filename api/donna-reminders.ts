...
// Folded in here (rather than its own api/donna-contacts.ts) for the same
// Vercel Hobby 12-function reason as the Chat tab/Files+School merges —
// Contacts and Reminders are also both frequent, lightweight personal-data
// CRUD, so sharing one function means real traffic to either keeps both warm.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.query.page === "contacts") {
    await handleContactsPage(req, res);
    return;
  }
